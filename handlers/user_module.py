...
async def click_weather_week(message: types.Message, state: FSMContext):  # , bot: Bot
    ...
    await message.answer(text=f'Если тебе не хочется мониторить постоянно погоду, я могу это делать за тебя и '
                              f'присылать оповещения, только, если прогноз погоды изменится свыше заданных'
...
async def get_city_weather_trigger(message: types.Message, state: FSMContext, bot: Bot):
    ...
    await message.answer(text=f'Где отслеживать, я понял ({city_name_no_translate}).'                             
        ...
        btns={ 'ТЕМПЕРАТУРА': 'trigger_temperature',

                'ТЕМПЕРАТУРА': 'trigger_temperature',
              'АТМОСФЕРНОЕ ДАВЛЕНИЕ': 'trigger_pressure',
              'ВЛАЖНОСТЬ': 'trigger_humidity',
              'СКОРОСТЬ ВЕТРА': 'trigger_wind_speed',
              'ОБЛАЧНОСТЬ': 'trigger_clouds',
              # 'Все параметры': 'trigger_all_params',
              ' ОТМЕНА': 'go_back',
              ' ДАЛЕЕ': 'trigger_clouds'}, sizes=(1, 2, 2, 2, ))
                                     )



    # Создать обработк с множеством келбеков на вход ( только те параметры с кнопок)
    # далее когда нажимаем кнопки они добавляются в список, а сообщение изменяется записывая на баннер список этих
    # параметров.
...
# Принимаем параметры триггера
@user_router.callback_query(StateFilter(InputUser.get_city_trigger), F.data.startswith('trigger'))
async def get_param_trigger(message: types.Message, state: FSMContext, bot: Bot):

    ...


    # Исключить повторное добавление

    # Как добавлять? в список кортеж или соловарь
